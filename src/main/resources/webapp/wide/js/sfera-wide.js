...
Sfera.Wide = function(config) {
    ...
    this.boot = function () {
        ...
        // doc viewer
        docViewer = addApp(new Sfera.Wide.Apps.DocViewer());
...
Sfera.Wide.Apps.DocViewer = function() {
    this.id = "dv";
    ...
    function onList(list, path) {
        ...
        for (var i=0; i<sub.length; i++) {
            if (sub[i].sub) {
                ...
                lQueue.push({name:n,
                    path:"webapp/cache/wide/doc/_templates/" + n + "." + ext});
            }
        }
        if (!path)
            processLoadQueue();
    ...
    this.startGenerator = function () {
        ...
        templates = {};
        files.load("list", ["webapp/cache/wide/doc/_templates",1], 100, onList);
    }
    ...
    function processWriteQueue() {
        ...
        if (!wQueue.length) {
            ...
            var f = wQueue.pop();
            // /src/main/javadoc/webapp/
            files.load("writefile", ["/webapp/cache/wide/doc/" + f.path, f.content], 10, processWriteQueue);
        }
    }
...
//# sourceMappingURL=sfera-wide.js.map